        ...
        <div className="flex flex-col items-center justify-center pt-5 pb-6 w-full h-full">
          ...
          {isDragActive ? (
            ...
            <>
              ...
              <p className="text-gray-400 text-sm">CSV</p>
            </>
          )}
        </div>
      </label>
...
const HasFile = ({ file, removeFile }: HasFileProps) => {
  return (
    <div className="w-1/2 p-4 h-full rounded-lg border-dashed border-4 border-gray-600 bg-gray-700 flex justify-center items-center">
      <div className="bg-white w-full rounded-md shadow-md flex gap-3 items-center justify-center">
        <FileIcon className="w-5 h-5 my-4 ml-4 text-gray-700" />
        <span className="text-sm text-gray-500 my-4">{file?.name}</span>
        <button type="button" onClick={removeFile} className="cursor-pointer ml-2 text-gray-700">
